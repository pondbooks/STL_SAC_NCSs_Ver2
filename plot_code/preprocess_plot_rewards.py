...
for i in range(14):
    data_nopreprocess = np.loadtxt('tau_delta_nopreprocess/returns'+str(i)+'.csv',skiprows=1,delimiter=",") 
    data_nopreprocess_temp = data_nopreprocess[:,1]
    data_nopreprocess_temp = np.array([data_nopreprocess_temp])
    data_proposed_method = np.loadtxt('tau_delta_preprocess/returns'+str(i)+'.csv',skiprows=1,delimiter=",")
...
plt.grid()
#plt.show()
plt.savefig("returns_preprocess.png")